#!/usr/bin/env python3
"""
Script de Extração e Upload - JUNTA MISSIONÁRIA DE PINHEIROS
Sistema PROVER -> Google Cloud Storage

Este script automatiza:
1. Login no sistema PROVER
2. Seleção da Junta Missionária de Pinheiros
3. Download dos lançamentos financeiros (CSV)
4. Upload para GCS na pasta bronze/junta_missionaria/
...
def main():
    """
    Função principal que executa o processo completo de extração e upload
    """
    logger.info("="*80)
    logger.info("INICIANDO PROCESSO DE EXTRAÇÃO E UPLOAD - JUNTA MISSIONÁRIA")
    logger.info("="*80)
    logger.info(f"Data/Hora: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Bucket GCS: {settings.gcs_bucket_name}")
    logger.info(f"Instituição: JUNTA MISSIONÁRIA DE PINHEIROS")
    logger.info("="*80)
    ...
    try:
        ...
        # Inicializar scraper (apenas Junta Missionária)
        scraper = ProverScraper()
        scraper.instituicoes = ["JUNTA MISSIONÁRIA DE PINHEIROS"]
        
        logger.info("Configurando driver e fazendo login...")
        scraper.setup_driver()
        scraper.login()
        
        logger.info("Selecionando instituição...")
        ...
        arquivos = scraper.baixar_lancamentos_financeiros("JUNTA MISSIONÁRIA DE PINHEIROS", data_inicio, data_fim)
        
        # Fechar o scraper
        scraper.close()
        
        # Montar resultados no formato esperado
        resultados = {"JUNTA MISSIONÁRIA DE PINHEIROS": arquivos}
